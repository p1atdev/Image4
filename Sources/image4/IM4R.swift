...
public class IM4R: DERSerializable {
    ...
    internal init(node: ASN1Node) throws {
        try DER.sequence(node, identifier: .sequence) { (scanner: inout ASN1NodeCollection.Iterator) in
            guard let head = scanner.next(), (try? String(ASN1IA5String(derEncoded: head))) == "IM4R" else {
    ...
    private func parseProperty(_ node: ASN1Node) throws -> ManifestProperty {
        ...
        guard let seqNode = scanner.next(), seqNode.identifier == .sequence else { throw Image4Error.invalidData }
        
        return try DER.sequence(seqNode, identifier: .sequence) { (scanner: inout ASN1NodeCollection.Iterator) in
            guard let fourccNode = scanner.next(),
                  let fourcc = try? String(ASN1IA5String(derEncoded: fourccNode)),
                  let valueNode = scanner.next() else {
                throw Image4Error.invalidData
            }
