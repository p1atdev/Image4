...
enum IPSWResource {
    static let LZSS_DEC_IPSW = "https://updates.cdn-apple.com/2021FallFCS/fullrestores/002-03194/8EC63AF9-19BE-4829-B389-27AECB41DD6A/iPhone_5.5_15.0_19A346_Restore.ipsw"
    static let LZSS_ENC_IPSW = "http://appldnld.apple.com/iOS9.3.5/031-73130-20160825-6A2C2FD4-6711-11E6-B3F4-173834D2D062/iPhone8,2_9.3.5_13G36_Restore.ipsw"
    static let LZFSE_IPSW = "https://updates.cdn-apple.com/2021FallFCS/fullrestores/002-02910/AF984499-D03A-43E7-9472-6D16BA756E5E/iPhone10,3,iPhone10,6_15.0_19A346_Restore.ipsw"
    static let PAYP_IPSW = "https://updates.cdn-apple.com/2022FallFCS/fullrestores/032-11449/3995EAD9-8F0E-491E-BEC8-C97B6B1845C7/iPhone15,3_16.1.2_20B110_Restore.ipsw"

    private static let client = HTTPClient(eventLoopGroupProvider: .singleton)
    ...
    static func getIPSWFile(url: String, filename: String) async throws -> Data {
        ...
        let safeUrl = url.replacingOccurrences(of: "[:/\\.]", with: "_", options: .regularExpression)
        let cacheFile = cacheDir.appendingPathComponent("\(safeUrl)_\(filename.replacingOccurrences(of: "/", with: "_"))")

        if FileManager.default.fileExists(atPath: cacheFile.path) {
            return try Data(contentsOf: cacheFile)
        }

        let remoteZip = try await RemoteZip(url: url, client: client)
    ...
    static var decLZSSIM4P: Data {
        get async throws { try await getIPSWFile(url: LZSS_DEC_IPSW, filename: "kernelcache.release.n66") }
    }

    static var encLZSSIM4P: Data {
        get async throws { try await getIPSWFile(url: LZSS_ENC_IPSW, filename: "kernelcache.release.n66") }
    }

    static var decLZFSEIM4P: Data {
        get async throws { try await getIPSWFile(url: LZFSE_IPSW, filename: "kernelcache.release.iphone10b") }
    }

    static var encLZFSEIM4P: Data {
        get async throws { try await getIPSWFile(url: LZFSE_IPSW, filename: "Firmware/dfu/iBSS.d22.RELEASE.im4p") }
    }

    static var paypIM4P: Data {
        get async throws { try await getIPSWFile(url: PAYP_IPSW, filename: "Firmware/dfu/iBSS.d74.RELEASE.im4p") }
    }
}
...
struct RemoteZip {
    ...
    init(url: String, client: HTTPClient) async throws {
        self.url = url
        self.client = client
        ...
        guard let lengthStr = response.headers.first(name: "Content-Length"),
        ...
        if cdOffset == 0xFFFFFFFF || cdSize == 0xFFFFFFFF {
            ...
            if locatorOffset >= 0 && 
                endData[locatorOffset] == 0x50 && endData[locatorOffset+1] == 0x4b && 
                endData[locatorOffset+2] == 0x06 && endData[locatorOffset+3] == 0x07 {
                
                let zip64EOCDOffset = Int64(endData.readUInt64(at: locatorOffset + 8))
                
                let zip64EOCDData = try await fetchRange(start: zip64EOCDOffset, end: zip64EOCDOffset + 56 - 1)
                if zip64EOCDData[0] == 0x50 && zip64EOCDData[1] == 0x4b && 
    ...
    func read(filename: String) async throws -> Data {
        ...
        let dataWithHeader = try await fetchRange(start: fetchStart, end: min(fetchEnd, fileSize - 1))
        ...
        let compressedData = dataWithHeader.subdata(in: dataOffset..<(dataOffset + Int(entry.compressedSize)))
        
        if entry.compressionMethod == 0 {
            return compressedData
        } else if entry.compressionMethod == 8 {
            return try decompressDeflate(compressedData, uncompressedSize: Int(entry.uncompressedSize))
        } else {
    ...
    private func fetchRange(start: Int64, end: Int64) async throws -> Data {
        var request = HTTPClientRequest(url: url)
        request.headers.add(name: "Range", value: "bytes=\(start)-\(end)")
        let response = try await client.execute(request, timeout: .seconds(60))
        
        if response.status != .partialContent && response.status != .ok {
            throw RemoteZipError.httpError(response.status.code)
        }
        
        var body = try await response.body.collect(upTo: 100 * 1024 * 1024)
    ...
    private func parseCentralDirectory(data: Data) throws -> [String: CentralDirectoryEntry] {
        ...
        while offset < data.count {
            ...
            if compressedSize == 0xFFFFFFFF || uncompressedSize == 0xFFFFFFFF || relativeOffset == 0xFFFFFFFF {
                ...
                while extraOffset + 4 <= extraEnd {
                    ...
                    if headerId == 0x0001 { // ZIP64 extra field
                        ...
                        if uncompressedSize == 0xFFFFFFFF && innerOffset + 8 <= extraEnd {
                            uncompressedSize = Int64(data.readUInt64(at: innerOffset))
                            innerOffset += 8
                        }
                        if compressedSize == 0xFFFFFFFF && innerOffset + 8 <= extraEnd {
                            compressedSize = Int64(data.readUInt64(at: innerOffset))
                            innerOffset += 8
                        }
                        if relativeOffset == 0xFFFFFFFF && innerOffset + 8 <= extraEnd {
